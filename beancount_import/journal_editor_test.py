...
def clean_entry(entry):
    ...
    if isinstance(entry, Transaction):
        entry = entry._replace(postings=list(map(clean_posting, entry.postings)))
    return entry
...
def check_journal_entries(editor: journal_editor.JournalEditor):
    new_editor = journal_editor.JournalEditor(editor.journal_path, editor.ignored_path)
...
def create_journal(
    tmpdir: py.path.local, contents: str, name: str = "journal.beancount"
):
...
""",
    ...
    assert test_util.format_entries(editor.entries) == test_util.format_entries(
        expected_entries
    )
...
""",
    ...
    new_entry2 = old_entry2._replace(
        postings=[
