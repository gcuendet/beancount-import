...
`beangulp.importer.Importer` subclasses importers.
...
class ImporterSource(DescriptionBasedSource):
    def __init__(
        self, directory: str, account: str, importer: Importer, **kwargs
    ) -> None:
    ...
    def _get_key_from_imported_entry(self, entry: Directive) -> Hashable:
        ...
        return (self.account, entry.date, source_posting.units, entry.narration)
